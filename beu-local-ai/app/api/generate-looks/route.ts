...
async function createRecommendations(
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a fashion recommendation engine. Return only valid JSON.'
      },
      {
        role: 'user',
        content: recommendationPrompt(analysis, occasion as any, feeling as any, environment as any)
      }
    ]
  });

  const parsed = safeJsonParse<{ recommendations: Omit<Recommendation, 'id' | 'moodboardImage'>[] }>(
...
export async function POST(req: NextRequest) {
  ...
  const { analysis, occasion, feeling, environment } = parsed.data;

  try {
    const recommendations = await createRecommendations(analysis, occasion, feeling, environment);
    ...
    return NextResponse.json({ analysis, recommendations: withImages });
  } catch (error: any) {
    console.error(error);
    return NextResponse.json({
